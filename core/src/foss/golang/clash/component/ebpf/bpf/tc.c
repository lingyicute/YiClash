...
SEC("tc_yiclashcore_redirect_to_tun")
int tc_tun_func(struct __sk_buff *skb) {
    ...
    __u32 key = 0, *yiclashcore_mark, *tun_ifindex;

    yiclashcore_mark = bpf_map_lookup_elem(&tc_params_map, &key);
    if (!yiclashcore_mark)
        return TC_ACT_OK;

    if (skb->mark == *yiclashcore_mark)
        return TC_ACT_OK;
